# RKF-AK3-REPO
[![Releases](https://img.shields.io/badge/Releases-View-blue)](https://github.com/Bacillusf/RKF-AK3-REPO/releases)
[![STAR](https://img.shields.io/github/stars/Bacillusf/RKF-AK3-REPO?style=flat&logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%2CPHN2ZyByb2xlPSJpbWciIHZpZXdCb3g9IjAgMCAyNCAyNCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48dGl0bGU%%2BPC9zdmc%2B)](https://github.com/Bacillusf/RKF-AK3-REPO/stargazers)
[![License](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)

为 ReKernelFlasher 软件提供云端 AnyKernel3 资源。

## 介绍

1. 本仓库主要为 RKF 软件的云 GKI 功能提供资源支持。
2. 本仓库使用 [cctv18](https://github.com/cctv18) 作者的 GitHub Actions 工作流进行编译，提供[ReSukiSU](https://github.com/ReSukiSU/ReSukiSU) [KernelSU-Next](https://github.com/KernelSU-Next/KernelSU-Next/) [KernelSU](https://github.com/tiann/KernelSU)所对应的欧加真AnyKernel3包
3. 刷入规则:***只要内核版本相同就可以刷入***，不需要注意编译时/Realease中提示的机型，其中展示的机型是说明源码所采用的机型，高通机型选择内核版本相同且不带有`MTK`标识的包，天玑机型则选择内核版本相同带有`MTK`标识的包
**Release命名**:OORB-6.6为例，`OORB`为`OPPO-ONEPLUS-Realme-Build`缩写，后方为内核大版本

## 注意

1. 刷入 AnyKernel3 前，***请务必备份 `boot.img`，以便在无法开机时进行救砖操作。***
2. 请根据您的机型选择适合的刷机包。若因选错包导致设备出现问题，本仓库概不负责。


### 备注

1. 如果您也希望将自己有效的 AnyKernel3 包上传至本仓库供他人使用，欢迎联系我。
2. 欢迎积极提交 issues 或反馈建议。。

